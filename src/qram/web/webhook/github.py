...
class GithubHandler(EventHandler):
    ...
    def handle_pr_comment(self, event: 'PrCommentEvent') -> Result[bool, ExpectedError]:
        ...
        split = event.message.strip().split()
        assert split[0] == '!qram'
        if len(split) == 2 and split[1] == 'go':
            # FIXME: fetch hangs if repos were not initialized, debug why
            # FIXME: hanged fetch hangs whole webserver as well! ffs tornado whyyy
